...
function friedmann!(du,u,p,z)
    ...
    Ω_m = 1 - Ω_ϕ(dQ, E, V(Q, p), z) # 8pi/3 * (0.5*((1+z)*dQ)^2 + V(Q, p)[1]/E^2)
    dE = 1.5*(E/(1+z)) * (Ω_m + 8pi/3 * ((1+z)*dQ)^2)

    du[1] = dQ
    du[2] = (2/(1+z) - dE/E) * dQ - dV(Q, p)/(E*(1+z))^2
    du[3] = dE
    du[4] = 1/E
end
...
cb = function(p, l, pred)
    ...
    println("Dark matter density parameter: ", 1 - Ω_ϕ(pred[2,1], pred[3,1], potential, uniquez[1])[1])
    return l < 1.05
end
...
plot4 = Plots.plot(uniquez, 1 .- Ω_ϕ(res[2,:], res[3,:], potential, uniquez), title="Density Evolution", xlabel="redshift z", ylabel="density parameter Ω", label="Ω_m")
plot4 = Plots.plot!(plot4, uniquez, Ω_ϕ(res[2,:], res[3,:], potential, uniquez), legend=:topright, label="Ω_ϕ")

println("Cosmological parameters: ")
println("Dark matter density parameter Ω_m = ", 1 - Ω_ϕ(res[2,:], res[3,:], potential, uniquez)[1])
println("Initial conditions for quintessence field = ", result.minimizer[1:2])
println("Slowroll satisfied for ϵ and η: ", slowroll)

m_ϕ =  Zygote.hessian(Q -> V(Q, result.minimizer[3:end]), res[1,1])[1]
