...
@time Threads.@threads for rep in 1:repetitions
    ...
    # Defining the potential and its gradient
    V(x, p) = p[1] * (1.0 + cos(x*p[2])) # other potentials are possible here
    dV(x, p) = Zygote.gradient(x -> V(x, p), x)[1]

    # Initialize parameters of the model with uniform distributions
    p = vcat(2.0*rand(Float64, ) .- 1.0, rand(Float64,1) .- 0.5)
    u0 = vcat(p[1:2], [1.0, 0.0])
    ps = vcat(p, 0.05.*rand(Float64,1), 3.0.*rand(Float64,1))
...
println("Parameters of the potential: ")
println("μ4 = ", Vfactor.*mean_params[3]./1e-16, " ± ", Vfactor.*std_params[3]./1e-16)
println("α = ", (1e-3*eVmpl).*mean_params[4]./ϕfactor, " ± ", (1e-3*eVmpl).*std_params[4]./ϕfactor)
...
# Save the figure
savefig(result_plot, "1024_sample_FreezingQuintessence.pdf")
