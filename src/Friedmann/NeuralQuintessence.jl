...
p = rand(Float32, 3)
u0 = vcat(p[1:2], [H0, 0.0]) # [phi, dphi, H, d_L]
tspan = (0.0, 7.0)
...
function friedmann!(du,u,p,z)
    ...
    Ω_m = p[1]*(1+z)^3
    dH = 1.5*H0^2/(H*(1+z))*(Ω_m + (H*(1+z)*dϕ)^2/ρ_c)

    du[1] = dϕ
    du[2] = -dϕ*((1+z)*dH - 2*H + dH/H + 1/(1+z)) - 1/(H*(1+z))*dV(ϕ, p[2:end])[1]
    du[3] = dH
    du[4] = c/H
end
...
pot = map(x -> Qtils.integrateNN(dV, x, result.minimizer[4:end]), res[1,:])
EoS = Qtils.calculateEOS(pot, res[2,:])
slowroll = Qtils.calculateslowroll(dV, result.minimizer[4:end], res[1,:], G)
...
resultplot = Plots.plot(plot1, plot2, plot3, layout=(1, 3), legend=:bottomright, size=(1200, 800))
