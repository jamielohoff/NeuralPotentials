...
function predict(params)
    return Array(solve(problem, Tsit5(), u0=vcat([H0],jparams[1:2],[0.0]), p=params[3:end], saveat=uniquez))
end
...
pot = map(x -> MyUtils.integrateNN(dV, x, result.minimizer[4:end]), res[2,:])
...
println("Mass parameter omega_m = ", result.minimizer[3])
