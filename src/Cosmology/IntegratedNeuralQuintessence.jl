...
function loss(params)
    ...
    # + slowrollregulatisation(pred[2,:], pred[3,:], pred[4,:], uniquez)
    return Qtils.reducedχ2(μ, averagedata, size(params,1)) + slowrollregulatisation(pred[2,:], pred[3,:], pred[4,:], uniquez), pred
end
...
EoS_plot = plot(uniquez, EoS, 
                        title="Equation of State", 
                        xlabel="redshift z", 
                        ylabel="equation of state w", 
                        legend=:topright)
V_plot = plot(uniquez, potential, 
                    title="Potential", 
                    xlabel="reshift z", 
                    ylabel="potential V", 
                    legend=:bottomright)
Ω_plot = plot(uniquez, 1 .- Ω_ϕ(res[2,:], res[4,:], potential, uniquez), 
