...
phispan = (0.0, 2*pi)
phi = range(phispan[1], phispan[2], length=100)
...
problem = ODEProblem(kepler!, true_u0, phispan, true_p)
...
cb = function(p,l,pred)
    ...
    traj_plot = plot(R .* cos.(phi), R .* sin.(phi), xlims=(-3.0, 3.0), ylims=(-3.0, 3.0))
